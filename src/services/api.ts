...
export const createVisit = async (payload: VisitProps): Promise<any> => {
  try {
    ...
    throw new Error(
      error.response?.data?.detail || 'Error creating visit record'
    );
  }
};
...
export const biDirectionalSyncVisits = async (useSessionStorage = true) => {
  try {
    ...
    const backendVisits = backendResponse.data;
    ...
    const visitKey = (v: Visit) => `${v.item_id}-${v.item_type}-${v.timestamp}`;

    // Map keys of backend visits for quick lookup
    const backendKeys = new Set(backendVisits.map(visitKey));

    // Filter frontend visits that do NOT exist on backend
    const newFrontendVisits = frontendVisits.filter(
      (v) => !backendKeys.has(visitKey(v))
    );

    // 4. Save new frontend visits to backend (POST requests)
    // Use Promise.all to parallelize
    await Promise.all(
      newFrontendVisits.map((visit) =>
    ...
    // 5. Merge all visits (backend + new frontend visits)
    const mergedVisits = [...backendVisits];
    newFrontendVisits.forEach((visit) => mergedVisits.push(visit));
